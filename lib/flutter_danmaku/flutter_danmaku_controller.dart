...
import 'flutter_danmaku_bullet_manager.dart';
import 'flutter_danmaku_render.dart';
...
class FlutterDanmakuController {
  ...
  /// 修改文字大小
  void changeBulletTextSize(int size) {
    assert(size > 0);
    FlutterDanmakuConfig.bulletTextSize = size.toDouble();
  ...
  FlutterDanmakuTrack? _findAllowInsertBottomTrack(Size bulletSize) {
    ...
    for (int i = tracks.length - 1; i >= tracks.length - 3; i--) {
      // 从当前的弹幕里找 有没有在这个轨道上的
      FlutterDanmakuBulletModel? bullet = _bulletManager.bottomBullets
