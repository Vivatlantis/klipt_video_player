...
class FlutterDanmakuUtils {
  ...
  static Size getDanmakuBulletSizeByText(String text) {
    ...
    RenderParagraph renderParagraph = RenderParagraph(
      TextSpan(
        text: text,
        style: TextStyle(
          fontSize: FlutterDanmakuConfig.bulletTextSize,
        ),
      ),
    ...
    double textwidth = renderParagraph
        .getMinIntrinsicWidth(FlutterDanmakuConfig.bulletTextSize)
        .ceilToDouble();
    double textheight =
  ...
  static double getBulletEveryFramerateRunDistance(double bulletWidth) {
    ...
    return FlutterDanmakuConfig.baseRunDistance +
        (bulletWidth / FlutterDanmakuConfig.everyFramerateRunDistanceScale);
  }

  // 算轨道相对可用区域是否溢出
  static bool isEnableTrackOverflowArea(FlutterDanmakuTrack track) =>
      track.offsetTop + track.trackHeight > FlutterDanmakuConfig.showAreaHeight;

  // 轨道注入子弹是否会碰撞
  static bool trackInsertBulletHasBump(
      FlutterDanmakuBulletModel trackLastBullet, Size needInsertBulletSize,
      {int? offsetMS = 0}) {
    // 是否离开了右边的墙壁
    ...
    double willInsertBulletRunDistance = offsetMS == null
        ? 0
        : (offsetMS.abs() / FlutterDanmakuConfig.unitTimer) *
  ...
  // 偏移子弹是否有空间能插入
  static hasInsertOffsetSpaceComputed(FlutterDanmakuBulletModel trackLastBullet,
      double willInsertBulletRunDistance) {
  ...
  static double remainderTimeLeaveScreen(
    ...
    double remanderDistance =
        (FlutterDanmakuConfig.areaSize.width + textWidth) - runDistance;
